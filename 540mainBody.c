...
void setBodyUniforms(uint32_t imageIndex) {                  // row 3
    ...
    bodySetUniforms(&body1, &aligned, 0);
    /* The water uses a trivial isometry and the second texture. */
    bodySetUniforms(&body2, &aligned, 1);
    ...
    bodySetUniforms(&body3, &aligned, 2);
...
int initializeCommandBuffers() {
    ...
    for (size_t i = 0; i < swap.numImages; i += 1) {
        ...
        bodyRender(&body1, &connCommandBuffers[i], &connPipelineLayout, &(desc.descriptorSets[i]), &aligned, 0);
        /* New: Render another vesh with another body UBO. */
        bodyRender(&body2, &connCommandBuffers[i], &connPipelineLayout, &(desc.descriptorSets[i]), &aligned, 1);
        /* New: Render another vesh with another body UBO. */
        bodyRender(&body3, &connCommandBuffers[i], &connPipelineLayout, &(desc.descriptorSets[i]), &aligned, 2);
