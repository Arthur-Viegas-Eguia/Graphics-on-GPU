...
void veshGetStyle(
    ...
    *bindingDesc = bindDesc;
    ...
    for (int i = 0; i < numAttr; i += 1)
        attrDescs[i] = veshGetAttributeDescription(i, numAttr, attrDims);
...
int veshInitializeMesh(veshVesh *vesh, meshMesh *mesh) {
    ...
    veshInitializeVertexBuffer(&vesh->vertBuf, &vesh->vertBufMem, vesh->attrDim, vesh->vertNum, mesh->vert);
    veshInitializeIndexBuffer(&vesh->triBuf, &vesh->triBufMem, vesh->triNum, mesh->tri);
    return 0;
}
